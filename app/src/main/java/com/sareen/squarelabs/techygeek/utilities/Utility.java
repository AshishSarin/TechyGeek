package com.sareen.squarelabs.techygeek.utilities;

/**
 * Created by ashish on 6/13/17.
 */

public class Utility
{
    // Constants used for passing data
    public static final String DETAIL_TTITLE = "detailTitle";
    public static final String DETAIL_TEXT = "detailText";
    public static final String DETAIL_POSTID = "detailPostID";
    public static final String DETAIL_IMAGE = "detailImage";


    // Constant for determining activity which calle detail activity
    public static final String DETAIL_CALLING_ACTIVITY = "calling_activity";


}
